...
import CustomSlider, {formatNumber} from "./CustomSlider";
...
function BusinessLoanApplication() {
    ...
    const handleAmountChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        const inputValue = e.target.value.replace(/[^0-9KkMm.]/g, "");
        let numValue: number;

        if (/[Kk]$/.test(inputValue)) {
            const baseValue = parseFloat(inputValue.replace(/[Kk]$/, ""));
            numValue = baseValue * 1000;
        } else if (/[Mm]$/.test(inputValue)) {
            const baseValue = parseFloat(inputValue.replace(/[Mm]$/, ""));
            numValue = baseValue * 1000000;
        } else {
        ...
        if (!isNaN(numValue)) {
            if (numValue > 10000000) {
                setHasError(true);
                setErrorMessage("Maximum loan amount is £10M");
            ...
            setLoanAmount(Math.round(numValue));
        }
    };
        ...
        <div className="bg-[#EDEDED] z-50 w-full">
            <div className="border border-gray-200 shadow-lg bg-white rounded-xl p-10 w-full">
              ...
              <span className="absolute left-3 top-1/2 transform -translate-y-1/2 text-teal-600 font-semibold">
                ...
                <button
                    className="w-full bg-teal-500 hover:bg-teal-600 text-white font-semibold py-3 px-6 rounded-lg transition-colors duration-200 flex items-center justify-center gap-4 cursor-pointer">
                    <p>Get Started</p>
