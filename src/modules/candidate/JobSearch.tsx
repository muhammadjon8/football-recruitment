            ...
            filteredJobs.map(job => (
              <div key={job.id} className="bg-white rounded-xl shadow-sm p-6 flex flex-wrap items-center gap-6">
                ...
                <div className="text-right min-w-[140px]">
                  <div className="text-gray-400 text-xs">Posted: {job.date}</div>
                  <button className="rounded-lg px-5 py-2 bg-blue-600 text-white font-semibold mt-3 hover:bg-blue-700 transition" onClick={() => alert('Job details (placeholder)')}>More Details</button>
                </div>
              </div>
            ))
          )}
